...
import { User, LogOut, Mail, Edit, Award, Building2 } from "lucide-react";
...
export default function EquityProfile() {
  ...
  return (
    <div className="min-h-screen bg-transparent flex flex-col items-center justify-between px-4 py-0">
        ...
        <div className="relative flex flex-col items-center">
          ...
          {user.image ? (
            <img
              src={user.image}
              alt={user.name}
              className="w-32 h-32 rounded-full border-4 border-[#a78bfa] object-cover shadow-lg mb-4"
            />
          ) : (
            <div className="w-32 h-32 rounded-full bg-gradient-to-br from-[#a78bfa] to-[#6d28d9] flex items-center justify-center text-5xl font-bold text-white border-4 border-[#a78bfa] shadow-lg mb-4">
          ...
          {/* Role Badge */}
          {user.role && (
            <span className="bg-[#a78bfa] text-black text-xs font-semibold px-4 py-1 rounded-full uppercase tracking-wide">{user.role}</span>
          )}
        </div>
      </div>
      {/* Email Card Only */}
