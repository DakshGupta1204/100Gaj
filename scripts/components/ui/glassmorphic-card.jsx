...
export const GlassmorphicCard = ({ name, role, image, properties, rating }) => {
    const [isHovered, setIsHovered] = useState(false);
    return (<motion.div className="w-64 rounded-xl overflow-hidden backdrop-blur-md bg-gray-100/50 border border-gray-300/30 shadow-xl" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} viewport={{ once: true }} onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)} whileHover={{
            y: -10,
      ...
      <motion.div className="p-4 bg-white/10 backdrop-blur-lg" initial={{ height: 0, opacity: 0 }} animate={{
