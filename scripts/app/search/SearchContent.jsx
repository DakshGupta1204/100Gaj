...
import { FaSearch, FaBed, FaBath, FaFilter, FaTimes, FaMapMarkerAlt, FaArrowRight, FaMap, FaList, FaNewspaper, FaRuler, FaBuilding, FaClock, } from "react-icons/fa";
...
const SearchPage = () => {
    ...
    const [filterTab, setFilterTab] = useState('properties');
    ...
    const [newlyAddedProperties, setNewlyAddedProperties] = useState(new Set());
    ...
    useEffect(() => {
        ...
        if (filterParam === "sale") {
            setSelectedFilters((prev) => (Object.assign(Object.assign({}, prev), { status: ["sale"] })));
        }
        else if (filterParam === "rent") {
            setSelectedFilters((prev) => (Object.assign(Object.assign({}, prev), { status: ["rent"] })));
        }
        ...
        if (propertyTypeParam) {
            setSelectedFilters((prev) => {
                ...
                if (!prev.propertyType.includes(propertyTypeParam)) {
                    return Object.assign(Object.assign({}, prev), { propertyType: [...prev.propertyType, propertyTypeParam] });
                }
                return prev;
            });
        }
        ...
        try {
            ...
            if (response.data.success) {
                ...
                if (typeof window !== "undefined") {
                    try {
                        sessionStorage.setItem("cachedProperties", JSON.stringify(properties));
                        sessionStorage.setItem("propertiesCacheTime", new Date().toString());
                    }
                    catch (_e) {
                    }
                }
            ...
            if (response.data.success) {
                ...
                if (typeof window !== "undefined") {
                    try {
                        ...
                        sessionStorage.setItem("projectsCacheTime", new Date().toString());
                    }
                    catch (_e) {
                    }
                }
    ...
    const propertyToSearchItem = (property) => ({
        _id: property._id,
    ...
    const projectToSearchItem = (project) => {
    ...
    const searchPropertiesInMapBounds = async () => {
        ...
        try {
            ...
            try {
                ...
                if (((_b = (_a = propertiesResponse === null || propertiesResponse === void 0 ? void 0 : propertiesResponse.data) === null || _a === void 0 ? void 0 : _a.properties) === null || _b === void 0 ? void 0 : _b.length) > 0) {
                    apiProperties = (propertiesResponse === null || propertiesResponse === void 0 ? void 0 : propertiesResponse.data.properties) || [];
                }
                // Handle projects response
                if (((_c = projectsResponse === null || projectsResponse === void 0 ? void 0 : projectsResponse.data) === null || _c === void 0 ? void 0 : _c.success) && ((_d = projectsResponse.data.projects) === null || _d === void 0 ? void 0 : _d.length) > 0) {
    ...
    const isPropertyMatchingFilters = (property) => {
        var _a, _b;
        // Search query filter
        if (searchQuery && searchQuery.trim() !== "") {
            ...
            const matchesSearch = property.title.toLowerCase().includes(query) ||
                ...
                (((_b = property.address) === null || _b === void 0 ? void 0 : _b.state) &&
        ...
        if (selectedFilters.propertyType.length > 0 &&
            !selectedFilters.propertyType.some((type) => property.propertyType &&
        ...
        // Area filter
        if (property.area < selectedFilters.area[0] ||
            property.area > selectedFilters.area[1]) {
            return false;
        }
        // Price filter
        if (property.price < selectedFilters.priceRange[0] ||
    ...
    const isProjectMatchingFilters = (item) => {
        ...
        // Project type filter
        if (selectedProjectFilters.projectType.length > 0 &&
            item.projectType &&
            !selectedProjectFilters.projectType.some((type) => item.projectType && item.projectType.toLowerCase().includes(type.toLowerCase()))) {
            return false;
        }
        // Project stage filter
        if (selectedProjectFilters.projectStage.length > 0 &&
            item.projectStage &&
            !selectedProjectFilters.projectStage.includes(item.projectStage)) {
            return false;
        }
        // Construction status filter
        if (selectedProjectFilters.constructionStatus.length > 0 &&
        ...
        if (selectedProjectFilters.unitTypes.length > 0 &&
            item.unitTypes &&
            Array.isArray(item.unitTypes)) {
            const hasMatchingUnitType = item.unitTypes.some((unitType) => {
        ...
        if (selectedProjectFilters.possessionYear.length > 0 &&
            item.possessionDate) {
        ...
        if (selectedProjectFilters.area[0] > 0 ||
            selectedProjectFilters.area[1] < 10000) {
            if (item.unitTypes && Array.isArray(item.unitTypes)) {
                const hasMatchingArea = item.unitTypes.some((unitType) => {
                    var _a, _b;
                    const minArea = ((_a = unitType.sizeRange) === null || _a === void 0 ? void 0 : _a.min) || 0;
                    const maxArea = ((_b = unitType.sizeRange) === null || _b === void 0 ? void 0 : _b.max) || 0;
                    return (minArea >= selectedProjectFilters.area[0] &&
                        maxArea <= selectedProjectFilters.area[1]) || (maxArea >= selectedProjectFilters.area[0] &&
        ...
        if (selectedProjectFilters.priceRange[0] > 0 ||
            selectedProjectFilters.priceRange[1] < 100000000) {
            if (item.unitTypes && Array.isArray(item.unitTypes)) {
                const hasMatchingPrice = item.unitTypes.some((unitType) => {
                    var _a, _b;
                    const minPrice = (((_a = unitType.priceRange) === null || _a === void 0 ? void 0 : _a.min) || 0) * 100000; // Convert lacs to rupees
                    const maxPrice = (((_b = unitType.priceRange) === null || _b === void 0 ? void 0 : _b.max) || 0) * 100000;
                    return (minPrice >= selectedProjectFilters.priceRange[0] &&
                        maxPrice <= selectedProjectFilters.priceRange[1]) || (maxPrice >= selectedProjectFilters.priceRange[0] &&
                ...
                if (item.price < selectedProjectFilters.priceRange[0] ||
    ...
    const fallbackToClientFiltering = () => {
        ...
        const itemsInBounds = allSearchItems.filter((item) => {
            ...
            if (item.type === 'property') {
                ...
                if (matches)
                    console.log('Property matches:', item.title);
                return matches;
            }
            ...
            if (item.type === 'project') {
                ...
                if (matches)
                    console.log('Project matches:', item.title);
                return matches;
            }
            return false;
        });
    ...
    const handleFilterChange = (filterType, value) => {
        setSelectedFilters((prev) => (Object.assign(Object.assign({}, prev), { [filterType]: value })));
    };
    const handleProjectFilterChange = (filterType, value) => {
        setSelectedProjectFilters((prev) => (Object.assign(Object.assign({}, prev), { [filterType]: value })));
    };
    const handleMapBoundsChange = (bounds) => {
        setMapBounds(bounds);
        // searchPropertiesInMapBounds();
        setSelectedFilters((prev) => (Object.assign(Object.assign({}, prev), { mapBounds: bounds })));
    };
    ...
    const filterSearchItems = useCallback(() => {
        ...
        const filtered = itemsToFilter.filter((item) => {
            ...
            if (item.type === 'property') {
                ...
                if (selectedFilters.propertyType.length > 0 &&
                    !selectedFilters.propertyType.some((type) => item.propertyType &&
                        item.propertyType.toLowerCase() === type.toLowerCase())) {
                    return false;
                }
                // Status filter (rent or sale) - check listingType property
                if (selectedFilters.status.length > 0 &&
                    !selectedFilters.status.some((status) => item.listingType === status)) {
                    return false;
                }
                ...
                if (typeof item.area === 'number') {
                    if (item.area < selectedFilters.area[0] ||
                ...
                if (typeof item.price === 'number') {
                    if (item.price < selectedFilters.priceRange[0] ||
    ...
    const categorizeArticle = (title, description) => {
        ...
        else if (content.includes("policy") ||
            content.includes("regulation") ||
            content.includes("law")) {
            return "Policy";
        }
        else if (content.includes("tips") ||
    ...
    useEffect(() => {
        if (typeof window !== "undefined") {
            try {
                const storedNewProperties = sessionStorage.getItem("newlyAddedProperties");
    ...
    useEffect(() => {
        if ((viewMode === 'map' || viewMode === 'split') && mapInstance) {
            ...
            const timeouts = [100, 300, 600, 1000].map(delay => setTimeout(() => {
                mapInstance.invalidateSize();
            }, delay));
            return () => {
    ...
    return (<div className="h-screen bg-black text-white overflow-hidden flex flex-col">
      <Navbar />
        ...
        <div className="container mx-auto px-0 md:px-4 py-3">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3 flex-1 px-4 md:px-0 md:ml-8 md:max-w-xl">
              <div className="relative flex-grow">
                <input type="text" value={searchQuery} onChange={(_e) => setSearchQuery(_e.target.value)} onKeyDown={(_e) => {
            if (_e.key === "Enter") {
                _e.preventDefault();
                filterSearchItems();
            }
        }} placeholder="Search location, property type..." className="w-full pl-4 pr-10 py-2 border border-gray-700 rounded-md focus:outline-none focus:ring-1 focus:ring-orange-500 text-black"/>
                <button type="button" onClick={handleSearch} className="absolute right-2 top-1/2 transform -translate-y-1/2">
                  <FaSearch className="h-4 w-4 text-gray-400"/>
                </button>
              </div>

              <button type="button" onClick={() => setShowFilters(true)} className="px-3 py-2 bg-orange-500 text-white rounded-md transition-colors flex items-center gap-1 hover:bg-orange-600">
          ...
          {(((selectedFilters.propertyType.length > 0 ||
            ...
            ((selectedProjectFilters.projectType.length > 0 ||
                ...
                selectedProjectFilters.area[1] < 10000) && (showingType === 'projects' || showingType === 'all'))) && (<div className="mt-3 flex flex-wrap items-center gap-2">
              <span className="text-xs text-gray-400">Active filters:</span>

              {/* Property Filters */}
              {(showingType === 'properties' || showingType === 'all') && (<>
                  {selectedFilters.status.includes("sale") && (<button onClick={() => {
                  ...
                  {selectedFilters.status.includes("rent") && (<button onClick={() => {
                  ...
                  {selectedFilters.propertyType.map((type) => (<button key={type} onClick={() => {
                  ...
                  {selectedFilters.beds.length > 0 && (<button onClick={() => handleFilterChange("beds", [])} className="px-2 py-1 bg-gray-800 text-white text-xs rounded-md flex items-center gap-1 border border-gray-700">
                      {selectedFilters.beds.join(", ")} BHK <FaTimes className="h-2 w-2"/>
                    </button>)}

                  {(selectedFilters.priceRange[0] > 0 ||
                    selectedFilters.priceRange[1] < 100000000) && (<button onClick={() => handleFilterChange("priceRange", [0, 100000000])} className="px-2 py-1 bg-gray-800 text-white text-xs rounded-md flex items-center gap-1 border border-gray-700">
                      Property ₹{selectedFilters.priceRange[0] / 100000}L - ₹
                      {selectedFilters.priceRange[1] / 100000}L <FaTimes className="h-2 w-2"/>
                    </button>)}

                  {(selectedFilters.area[0] > 0 || selectedFilters.area[1] < 10000) && (<button onClick={() => handleFilterChange("area", [0, 10000])} className="px-2 py-1 bg-gray-800 text-white text-xs rounded-md flex items-center gap-1 border border-gray-700">
              ...
              {(showingType === 'projects' || showingType === 'all') && (<>
                  {selectedProjectFilters.projectType.map((type) => (<button key={type} onClick={() => {
                  ...
                  {selectedProjectFilters.projectStage.map((stage) => (<button key={stage} onClick={() => {
                  ...
                  {selectedProjectFilters.constructionStatus.map((status) => (<button key={status} onClick={() => {
                  ...
                  {selectedProjectFilters.unitTypes.map((unit) => (<button key={unit} onClick={() => {
                  ...
                  {selectedProjectFilters.possessionYear.map((year) => (<button key={year} onClick={() => {
                  ...
                  {(selectedProjectFilters.priceRange[0] > 0 ||
                    selectedProjectFilters.priceRange[1] < 100000000) && (<button onClick={() => handleProjectFilterChange("priceRange", [0, 100000000])} className="px-2 py-1 bg-blue-800 text-white text-xs rounded-md flex items-center gap-1 border border-blue-700">
                  ...
                  {(selectedProjectFilters.area[0] > 0 || selectedProjectFilters.area[1] < 10000) && (<button onClick={() => handleProjectFilterChange("area", [0, 10000])} className="px-2 py-1 bg-blue-800 text-white text-xs rounded-md flex items-center gap-1 border border-blue-700">
                      Project {selectedProjectFilters.area[0]}-{selectedProjectFilters.area[1]} sqft{" "}
                      <FaTimes className="h-2 w-2"/>
                    </button>)}
                </>)}

              <button onClick={resetFilters} className="px-2 py-2 bg-orange-600 hover:text-orange-600 text-xs transition-colors font-medium rounded-lg">
                Reset all
              </button>
            </div>)}
        </div>
      ...
      <AnimatePresence>
        {showFilters && (<div className="fixed inset-0 bg-black/50 flex items-start justify-end z-[1000]" onClick={() => setShowFilters(false)}>
            <motion.div className="bg-black h-full w-full md:max-w-md overflow-y-auto shadow-orange-500 shadow-sm" initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "tween", duration: 0.3 }} onClick={(_e) => _e.stopPropagation()}>
              <div className="sticky top-0 bg-gradient-to-r from-orange-600 to-orange-500 z-10 border-b border-gray-700">
                <div className="flex items-center justify-between p-4 pb-0">
                  ...
                  <button className="p-1 rounded-full hover:bg-gray-700" onClick={() => setShowFilters(false)}>
                ...
                <div className="flex bg-black/20 rounded-lg mx-4 mt-3 mb-4 p-1">
                  <button onClick={() => setFilterTab('properties')} className={`flex-1 py-2 px-3 rounded-md text-sm font-medium transition-colors ${filterTab === 'properties'
                ? 'bg-white text-orange-600'
                : 'text-white hover:text-gray-200'}`}>
                    Properties
                  </button>
                  <button onClick={() => setFilterTab('projects')} className={`flex-1 py-2 px-3 rounded-md text-sm font-medium transition-colors ${filterTab === 'projects'
                ? 'bg-white text-orange-600'
                : 'text-white hover:text-gray-200'}`}>
                    Projects
                  </button>
              ...
              <div className="p-4 space-y-6">
                ...
                {filterTab === 'properties' && (<>
                    ...
                    handleFilterChange("status", newStatus);
                }} className={`px-4 py-2 rounded-md text-sm ${selectedFilters.status.includes("sale")
                    ...
                    handleFilterChange("status", newStatus);
                }} className={`px-4 py-2 rounded-md text-sm ${selectedFilters.status.includes("rent")
                    ? "bg-orange-500 text-white"
                    : "bg-gray-800 text-white hover:bg-gray-700 border border-gray-700"}`}>
                          For Rent
                        </button>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {propertyTypes.map((type) => (<button key={type} onClick={() => {
                        ...
                        handleFilterChange("propertyType", newTypes);
                    }} className={`px-3 py-2 rounded-md text-sm ${selectedFilters.propertyType.includes(type)
                        ? "bg-orange-500 text-white"
                        : "bg-gray-800 text-white hover:bg-gray-700 border border-gray-700"}`}>
                            {type}
                          </button>))}
                    ...
                    <div>
                      ...
                      <div className="flex gap-1">
                        {bedOptions.map((bed) => (<button key={bed} onClick={() => {
                    ...
                    }} className={`flex-1 py-2 rounded-md text-sm ${selectedFilters.beds.includes(bed)
                        ? "bg-orange-500 text-white"
                        : "bg-gray-800 text-white hover:bg-gray-700 border border-gray-700"}`}>
                            {bed} BHK
                          </button>))}
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                        {priceRanges.map((range, index) => (<button key={index} onClick={() => handleFilterChange("priceRange", range.value)} className={`w-full px-4 py-2 rounded-md text-left text-sm ${selectedFilters.priceRange[0] === range.value[0] &&
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                        {areaRanges.map((range, index) => (<button key={index} onClick={() => handleFilterChange("area", range.value)} className={`w-full px-4 py-2 rounded-md text-left text-sm ${selectedFilters.area[0] === range.value[0] &&
                ...
                {filterTab === 'projects' && (<>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {projectTypes.map((type) => (<button key={type} onClick={() => {
                        ...
                        handleProjectFilterChange("projectType", newTypes);
                    }} className={`px-3 py-2 rounded-md text-sm ${selectedProjectFilters.projectType.includes(type)
                        ? "bg-orange-500 text-white"
                        : "bg-gray-800 text-white hover:bg-gray-700 border border-gray-700"}`}>
                            {type}
                          </button>))}
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {projectStages.map((stage) => (<button key={stage} onClick={() => {
                        ...
                        handleProjectFilterChange("projectStage", newStages);
                    }} className={`px-3 py-2 rounded-md text-sm ${selectedProjectFilters.projectStage.includes(stage)
                        ? "bg-orange-500 text-white"
                        : "bg-gray-800 text-white hover:bg-gray-700 border border-gray-700"}`}>
                            {stage}
                          </button>))}
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {constructionStatuses.map((status) => (<button key={status} onClick={() => {
                        ...
                        handleProjectFilterChange("constructionStatus", newStatuses);
                    }} className={`px-3 py-2 rounded-md text-sm ${selectedProjectFilters.constructionStatus.includes(status)
                        ? "bg-orange-500 text-white"
                        : "bg-gray-800 text-white hover:bg-gray-700 border border-gray-700"}`}>
                            {status}
                          </button>))}
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {unitTypeOptions.map((unit) => (<button key={unit} onClick={() => {
                    ...
                    }} className={`px-3 py-2 rounded-md text-sm ${selectedProjectFilters.unitTypes.includes(unit)
                        ? "bg-orange-500 text-white"
                        : "bg-gray-800 text-white hover:bg-gray-700 border border-gray-700"}`}>
                            {unit}
                          </button>))}
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {possessionYears.map((year) => (<button key={year} onClick={() => {
                        ...
                        handleProjectFilterChange("possessionYear", newYears);
                    }} className={`px-3 py-2 rounded-md text-sm ${selectedProjectFilters.possessionYear.includes(year)
                        ? "bg-orange-500 text-white"
                        : "bg-gray-800 text-white hover:bg-gray-700 border border-gray-700"}`}>
                            {year}
                          </button>))}
                    ...
                    <div>
                      <h3 className="font-medium mb-3 text-white">Price Range</h3>
                      <div className="space-y-2">
                        {priceRanges.map((range, index) => (<button key={index} onClick={() => handleProjectFilterChange("priceRange", range.value)} className={`w-full px-4 py-2 rounded-md text-left text-sm ${selectedProjectFilters.priceRange[0] === range.value[0] &&
                    ...
                    <div>
                      <h3 className="font-medium mb-3 text-white">Area (sqft)</h3>
                      <div className="space-y-2">
                        {areaRanges.map((range, index) => (<button key={index} onClick={() => handleProjectFilterChange("area", range.value)} className={`w-full px-4 py-2 rounded-md text-left text-sm ${selectedProjectFilters.area[0] === range.value[0] &&
              ...
              <div className="sticky bottom-0 bg-black p-4 border-t border-gray-700 flex gap-3">
                <button onClick={resetFilters} className="flex-1 py-3 rounded-md bg-gray-800 text-white hover:bg-gray-700 border border-gray-700">
                  Reset
                </button>
                <button onClick={() => {
      ...
      <div className="md:hidden flex-shrink-0 z-20 bg-black py-2 px-4 flex justify-between border-b border-gray-800">
        ...
        <div className="flex">
          <button onClick={() => setViewMode('list')} className={`px-3 py-1.5 ${viewMode === 'list' ? 'bg-orange-500' : 'bg-gray-700'} text-white rounded-l-md text-sm font-medium flex items-center gap-1.5`}>
            <FaList className="h-3 w-3"/>
            <span>List</span>
          </button>
          <button onClick={() => setViewMode('map')} className={`px-3 py-1.5 ${viewMode === 'map' ? 'bg-orange-500' : 'bg-gray-700'} text-white rounded-r-md text-sm font-medium flex items-center gap-1.5`}>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 md:gap-4">
              <div className="flex bg-gray-800 rounded-lg p-1">
                <button onClick={() => setShowingType('all')} className={`px-2 md:px-4 py-1 md:py-2 rounded-md text-xs md:text-sm font-medium transition-colors ${showingType === 'all'
            ? 'bg-orange-500 text-white'
            : 'text-gray-300 hover:text-white'}`}>
                  All <span className="hidden sm:inline">({allSearchItems.length})</span>
                </button>
                <button onClick={() => setShowingType('properties')} className={`px-2 md:px-4 py-1 md:py-2 rounded-md text-xs md:text-sm font-medium transition-colors ${showingType === 'properties'
            ? 'bg-orange-500 text-white'
            : 'text-gray-300 hover:text-white'}`}>
                  Properties <span className="hidden sm:inline">({allSearchItems.filter(item => item.type === 'property').length})</span>
                </button>
                <button onClick={() => setShowingType('projects')} className={`px-2 md:px-4 py-1 md:py-2 rounded-md text-xs md:text-sm font-medium transition-colors ${showingType === 'projects'
            ? 'bg-orange-500 text-white'
            : 'text-gray-300 hover:text-white'}`}>
      ...
      <div className="flex flex-col md:flex-row flex-1 min-h-0">
        {/* Map Section */}
        <div id="map-container" className={`${viewMode === 'list' ? "hidden" : "block"} ${viewMode === 'split' ? "md:flex-1" : "flex-1"} h-full overflow-hidden relative`} style={{
            display: (viewMode === 'map' || (viewMode === 'split' && window.innerWidth >= 768)) ? 'block' : 'none'
        }}>
          <MapErrorBoundary>
            <Map center={mapCenter} zoom={zoom} onMapLoad={setMapInstance} properties={filteredSearchItems.map((item) => {
            ...
            if (item.type === 'property') {
                ...
                if (item.unitTypes && Array.isArray(item.unitTypes)) {
                    const minPrice = Math.min(...item.unitTypes.map((u) => u.priceRange.min));
                    const maxPrice = Math.max(...item.unitTypes.map((u) => u.priceRange.max));
            ...
            if (mapBounds) {
              ...
              {viewMode === 'map' && (<button onClick={() => setViewMode('list')} className="md:hidden bg-black/90 backdrop-blur-sm text-white px-3 py-1 rounded-md text-xs font-medium border border-gray-600 shadow-lg">
                  📋 View List
                </button>)}
            </div>
          </MapErrorBoundary>
        ...
        <div className={`${viewMode === 'map' ? "hidden" : "flex"} ${viewMode === 'split' ? "md:w-1/2" : "w-full"} bg-black flex-col min-h-0`} style={{
            ...
            <div className="p-3 md:p-6 min-h-full">
              ...
              <div className="flex items-center gap-3">
                <button onClick={() => setShowArticles(!showArticles)} className="flex items-center gap-1 text-sm text-orange-500 hover:text-orange-600 transition-colors">
            ...
            {!showArticles && (<>
                {isLoading ? (<div className="flex justify-center items-center py-12">
                  ...
                  </div>) : filteredSearchItems.length > 0 ? (<div className="space-y-3 md:space-y-4 pb-20">
                    {filteredSearchItems.map((item) => (<motion.div key={item._id} id={`item-${item._id}`} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className={`bg-gray-900 rounded-lg shadow-lg overflow-hidden hover:shadow-xl transition-all text-white border border-gray-800`}>
                        ...
                        if (item.type === 'property') {
                          ...
                          <div className="relative w-full md:w-2/5 h-48 md:h-auto">
                            <Image src={item.images && item.images.length > 0
                        ? item.images[0]
                        : "/placeholder-property.jpg"} alt={item.title} fill sizes="(max-width: 768px) 100vw, 40vw" className="object-cover" priority={false} onError={(e) => {
                        const target = e.target;
                        target.src = "/placeholder-property.jpg";
                    }}/>
                            <div className="absolute top-2 right-2">
                              {item.type === 'property' ? (<span className="bg-orange-600/90 backdrop-blur-sm text-white px-2 py-1 rounded text-xs font-medium shadow-sm">
                                  {item.listingType === "rent" ? "For Rent" : "For Sale"}
                                </span>) : (<span className="bg-blue-600/90 backdrop-blur-sm text-white px-2 py-1 rounded text-xs font-medium shadow-sm">
                                  Project
                                </span>)}
                            </div>
                            {newlyAddedProperties.has(item._id) && (<div className="absolute top-2 left-2">
                          ...
                          <div className="p-4 md:w-3/5 flex flex-col">
                            <div className="mb-auto">
                              ...
                              <div className="mt-3 flex items-center text-xs text-white/70 flex-wrap gap-x-3 gap-y-1">
                                {item.type === 'property' ? (<>
                                    {item.bedrooms && (<span className="flex items-center">
                                        <FaBed className="h-3 w-3 mr-1"/>
                                        {item.bedrooms} BR
                                      </span>)}
                                    {item.bathrooms && (<span className="flex items-center">
                                        <FaBath className="h-3 w-3 mr-1"/>
                                        {item.bathrooms} BA
                                      </span>)}
                                    {item.area && (<span className="flex items-center">
                                        <FaRuler className="h-3 w-3 mr-1"/>
                                        {typeof item.area === 'number' ? `${item.area} sqft` : item.area}
                                      </span>)}
                                  </>) : (<>
                                    {item.constructionStatus && (<span className="flex items-center">
                                        <FaBuilding className="h-3 w-3 mr-1"/>
                                        {item.constructionStatus}
                                      </span>)}
                                    {item.projectStage && (<span className="flex items-center">
                        ...
                        : (item.unitTypes && Array.isArray(item.unitTypes)
                            ...
                            : `₹${item.price ? (Number(item.price) / 100000).toFixed(1) : "N/A"} Lacs`)}
                                ...
                                {item.type === 'property' && item.listingType === "rent" && (<span className="text-xs text-white/60">/month</span>)}
                              </div>
                              <button onClick={(e) => {
                        e.stopPropagation();
                        if (item.type === 'property') {
                  ...
                  </div>) : (<div className="text-center py-12 px-4 bg-white rounded-md shadow-orange-500 shadow-sm border border-gray-700">
                    ...
                    <button onClick={resetFilters} className="px-4 py-2 bg-gradient-to-r from-orange-500 to-orange-600 text-white text-sm rounded-md hover:shadow-md">
                      Reset Filters
                    </button>
                  </div>)}
              </>)}
            ...
            {/* Articles Section */}
            {showArticles && (<div className="space-y-6">
                <div className="flex items-center justify-between mb-4">
                ...
                {articlesLoading ? (<div className="flex justify-center items-center py-12">
                    <div className="animate-spin rounded-full h-10 w-10 border-t-2 border-b-2 border-orange-500"></div>
                  </div>) : articlesError ? (<div className="text-center py-8 px-4 bg-white rounded-md shadow-orange-500 shadow-sm border border-gray-700">
                    <p className="text-gray-500">{articlesError}</p>
                  </div>) : (<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {articles.map((article) => (<motion.div key={article.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-white rounded-md shadow-orange-500 shadow-sm overflow-hidden border border-gray-700 flex flex-col">
                        <div className="relative h-40">
                          <Image src={article.urlToImage} alt={article.title} fill sizes="(max-width: 768px) 100vw, 50vw" className="object-cover"/>
                          <div className="absolute top-2 right-2">
                        ...
                        <div className="p-4 flex flex-col flex-grow">
                          ...
                          <div className="mt-auto flex items-center justify-between">
                            ...
                            <a href={article.url} target="_blank" rel="noopener noreferrer" className="px-3 py-1 bg-gray-800 text-white text-xs rounded-md hover:bg-gray-700 transition-colors flex items-center gap-1 border border-gray-700">
      ...
      <motion.div className="fixed bottom-1 left-0 right-0 z-50 flex justify-center items-center pb-4 md:pb-6 pointer-events-none" initial={{ y: 0, opacity: 1 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.4 }}>
        <div className="backdrop-blur-lg rounded-full shadow-lg border border-white/20 px-2 py-1.5 pointer-events-auto bg-orange-600 hover:bg-orange-800">
          <div className="flex space-x-1 relative">
            {navItems.map((item) => (<Link key={item.href} href={item.href} className={`relative px-2 py-1 text-base rounded-full transition-all duration-300 font-bold ${item.href === "/search" ? "text-white" : "text-white"}`}>
                {item.href === "/search" && (<motion.div className="absolute inset-0 bg-gradient-to-r from-black to-orange-500 rounded-full -z-10" layoutId="indicator" transition={{ type: "spring", duration: 0.6 }}/>)}
                {item.label}
              </Link>))}
          </div>
        </div>
    ...
    render() {
        if (this.state.hasError) {
          ...
          <div className="text-center p-8">
            ...
            <button onClick={() => this.setState({ hasError: false })} className="px-4 py-2 bg-orange-500 text-white rounded-md hover:bg-orange-600 transition-colors">
              Retry
            </button>
          </div>
        </div>);
