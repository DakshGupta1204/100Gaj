...
export async function GET(req) {
    try {
        ...
        const portfolio = await Portfolio.findOne({ userId }, "holdings")
            .populate("holdings.commercialPropertyId", "title address propertyType currentPrice totalPropertyValue")
            .lean();
        if (!portfolio) {
            return NextResponse.json({
                success: false,
        ...
        const enrichedHoldings = portfolio.holdings.map((holding) => {
            const gainLossPercentage = holding.totalInvested > 0
                ? ((holding.currentValue - holding.totalInvested) /
                    ...
                    100
                : 0;
            const totalReturn = holding.unrealizedGainLoss +
                holding.realizedGainLoss +
                holding.dividendsReceived;
            const totalReturnPercentage = holding.totalInvested > 0
                ? (totalReturn / holding.totalInvested) * 100
                : 0;
            return Object.assign(Object.assign({}, holding), { gainLossPercentage,
                totalReturn,
...
export async function POST(req) {
    try {
        ...
        if (!body.userId ||
            ...
            return NextResponse.json({
                success: false,
                message: "Missing required fields: userId, commercialPropertyId, shares, avgPurchasePrice",
            }, { status: 400 });
        }
        ...
        const existingHoldingIndex = portfolio.holdings.findIndex((holding) => holding.commercialPropertyId.toString() === body.commercialPropertyId);
        if (existingHoldingIndex !== -1) {
            ...
            portfolio.holdings[existingHoldingIndex] = Object.assign(Object.assign({}, existingHolding), { shares: newTotalShares, avgPurchasePrice: newAvgPurchasePrice, totalInvested: newTotalInvested, currentValue: existingHolding.currentValue + currentValue, lastPurchaseDate: purchaseDate });
        }
        else {
            // Add new holding
            const newHolding = {
        ...
        portfolio.summary.totalInvested = portfolio.holdings.reduce((sum, holding) => sum + holding.totalInvested, 0);
        portfolio.summary.currentValue = portfolio.holdings.reduce((sum, holding) => sum + holding.currentValue, 0);
        ...
        portfolio.summary.totalShares = portfolio.holdings.reduce((sum, holding) => sum + holding.shares, 0);
