...
export async function GET(req) {
    try {
        ...
        const portfolio = await Portfolio.findOne({ userId }, "performance summary totalReturnPercentage").lean();
        if (!portfolio) {
            return NextResponse.json({
                success: false,
        ...
        if (timeframe === "monthly") {
            performanceData = Object.assign(Object.assign({}, performanceData), { yearlyReturns: [] });
        }
        else if (timeframe === "yearly") {
            performanceData = Object.assign(Object.assign({}, performanceData), { monthlyReturns: [] });
        }
        // Calculate additional metrics
        const currentReturn = portfolio.summary.totalInvested > 0
