...
export const GET = withAuth(async (request, userId) => {
    try {
        ...
        return NextResponse.json({ error: 'An error occurred while fetching subscription details' }, { status: 500 });
    }
});
// POST /api/users/subscription - Create or update user subscription
export const POST = withAuth(async (request, userId) => {
    try {
        const { userType, planType, price, transactionId, autoRenew = false } = await request.json();
        // Input validation
        if (!userType || !planType || !transactionId) {
            return NextResponse.json({ error: 'User type, plan type, and transaction ID are required' }, { status: 400 });
        }
        // Check if user type is valid
        if (!Object.values(UserType).includes(userType)) {
            return NextResponse.json({ error: 'Invalid user type. Must be owner, dealer, or buyer' }, { status: 400 });
        }
        // Check if plan type is valid
        if (!Object.values(PlanType).includes(planType)) {
            return NextResponse.json({ error: 'Invalid plan type. Must be free, basic, standard, premium, or boss' }, { status: 400 });
        }
        ...
        if (planType === PlanType.FREE && planPrice !== 0) {
            ...
            if (planType === PlanType.BASIC && planPrice !== 500)
                validPrice = false;
            else if (planType === PlanType.STANDARD && planPrice !== 1000)
                validPrice = false;
            else if (planType === PlanType.PREMIUM && planPrice !== 2000)
                validPrice = false;
        }
        else if (userType === UserType.DEALER) {
            if (planType === PlanType.BASIC && planPrice !== 1000)
                validPrice = false;
            else if (planType === PlanType.STANDARD && planPrice !== 2000)
                validPrice = false;
            else if (planType === PlanType.PREMIUM && planPrice !== 5000)
                validPrice = false;
        }
        else if (userType === UserType.BUYER && planType === PlanType.BOSS) {
            if (![1000, 2000, 10000].includes(planPrice))
                validPrice = false;
        }
        if (!validPrice) {
            return NextResponse.json({ error: 'Invalid price for the selected plan and user type' }, { status: 400 });
        }
        await connectDB();
        ...
        return NextResponse.json({ error: 'An error occurred while processing subscription' }, { status: 500 });
    }
});
