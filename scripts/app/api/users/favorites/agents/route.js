...
export const GET = withAuth(async (request, userId) => {
    ...
    try {
        ...
        return NextResponse.json({
            favorites: ((_a = user.favorites) === null || _a === void 0 ? void 0 : _a.agents) || [],
            success: true
        });
    }
    catch (_error) {
        return NextResponse.json({ error: 'An error occurred while fetching favorite agents' }, { status: 500 });
    }
});
...
export const POST = withAuth(async (request, userId) => {
    try {
        ...
        const updatedUser = await User.findByIdAndUpdate(userId, { $addToSet: { 'favorites.agents': agentId } }, { new: true });
        if (!updatedUser) {
            return NextResponse.json({ error: 'User not found' }, { status: 404 });
        }
        return NextResponse.json({
        ...
        return NextResponse.json({ error: 'An error occurred while adding agent to favorites' }, { status: 500 });
    }
});
