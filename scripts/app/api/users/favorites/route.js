...
export const POST = withAuth(async (request, userId) => {
    try {
        ...
        if (existingFavorite) {
            return NextResponse.json({ error: 'Property is already in favorites' }, { status: 400 });
        }
        // Add to favorites
        ...
        return NextResponse.json({ error: 'An error occurred while adding to favorites' }, { status: 500 });
    }
});
