...
export const PATCH = withAuth(async (request, userId) => {
    try {
        ...
        const { subscriptionIds, action, extensionDays, reason } = await request.json();
        // Validate input
        ...
        if (!action || !['extend', 'disable', 'enable', 'reset_usage'].includes(action)) {
            return NextResponse.json({ error: 'Invalid action. Must be one of: extend, disable, enable, reset_usage' }, { status: 400 });
        }
        ...
        return NextResponse.json({ error: 'An error occurred while updating subscriptions' }, { status: 500 });
    }
});
