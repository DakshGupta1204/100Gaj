 ...
 */
async function getAgentAnalytics(request, userId) {
    try {
        await connectDB();
        ...
        return NextResponse.json({ error: 'An error occurred while generating agent analytics' }, { status: 500 });
    }
}
// Helper functions for analytics calculations
function calculateResponseRate(inquiries) {
...
function calculateAverageResponseTime(inquiries) {
    const respondedInquiries = inquiries.filter(inquiry => inquiry.respondedAt && inquiry.createdAt);
...
function calculateLeadConversionRate(properties, inquiries) {
    const soldProperties = properties.filter(p => p.status === 'sold').length;
    return inquiries.length > 0 ? Math.round((soldProperties / inquiries.length) * 100) : 0;
}
function calculatePropertyTypeDistribution(properties) {
    const distribution = {};
...
function getTopPerformingListings(properties, inquiries) {
...
function generateTimeSeriesData(inquiries, startDate, endDate, period) {
    let interval;
...
function calculatePotentialEarnings(properties) {
...
export const GET = withAuthAndSubscription(getAgentAnalytics, 'use_ai', // Using AI tools category for analytics
[UserType.DEALER], PlanType.PREMIUM // Requiring PREMIUM plan for advanced analytics
);
