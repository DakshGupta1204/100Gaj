...
function isTimeSlotAvailable(dayOfWeek, startTime, endTime, availability) {
...
export const GET = withAuth(async (request, userId) => {
    try {
        ...
        if (agentId !== userId) {
            ...
            appointments = appointments.filter((apt) => apt.client.toString() === userId);
        }
...
export const POST = withAuth(async (request, userId) => {
    ...
    try {
        ...
        const { date, startTime, endTime, propertyId, type, notes, } = await request.json();
        if (!agentId || !isValidObjectId(agentId)) {
            return NextResponse.json({ error: 'Invalid agent ID format' }, { status: 400 });
        }
        // Validate required fields
        if (!date || !startTime || !endTime || !type) {
            return NextResponse.json({ error: 'Date, start time, end time, and type are required' }, { status: 400 });
        }
        await connectDB();
        ...
        if (!agent) {
            return NextResponse.json({ error: 'Agent not found or not verified' }, { status: 404 });
        }
        // If property viewing, validate property
        if (type === 'property-viewing') {
            if (!propertyId || !isValidObjectId(propertyId)) {
                return NextResponse.json({ error: 'Valid property ID is required for property viewing' }, { status: 400 });
            }
            ...
            if (((_a = property.agent) === null || _a === void 0 ? void 0 : _a.toString()) !== agentId && property.owner.toString() !== agentId) {
                return NextResponse.json({ error: 'Agent is not associated with this property' }, { status: 403 });
            }
        }
        // Get agent's schedule
        const schedule = await AgentSchedule.findOne({ agent: agentId });
        if (!schedule) {
            return NextResponse.json({ error: 'Agent schedule not found' }, { status: 404 });
        }
        // Check if the appointment date is blocked
        const appointmentDate = new Date(date);
        const isDateBlocked = schedule.blockedDates.some((blockedDate) => blockedDate.date.toDateString() === appointmentDate.toDateString());
        if (isDateBlocked) {
            return NextResponse.json({ error: 'Agent is not available on this date' }, { status: 400 });
        }
        ...
        if (!isTimeSlotAvailable(dayOfWeek, startTime, endTime, schedule.availability)) {
            return NextResponse.json({ error: 'Time slot is outside agent\'s availability' }, { status: 400 });
        }
        // Check for conflicting appointments
        const hasConflict = schedule.appointments.some((apt) => {
            return (apt.date.toDateString() === appointmentDate.toDateString() &&
                apt.status !== 'cancelled' &&
                doTimeSlotsOverlap(startTime, endTime, apt.startTime, apt.endTime));
        });
        if (hasConflict) {
            return NextResponse.json({ error: 'Time slot conflicts with another appointment' }, { status: 400 });
        }
        // Create appointment
        const appointment = {
