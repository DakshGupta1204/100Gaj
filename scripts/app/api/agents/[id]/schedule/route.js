...
export async function GET(request, { params }) {
    try {
        ...
        return NextResponse.json({ error: 'An error occurred while fetching agent schedule' }, { status: 500 });
    }
}
...
export const PUT = withAuth(async (request, userId) => {
    try {
        ...
        if (id !== userId) {
            return NextResponse.json({ error: 'You can only update your own schedule' }, { status: 403 });
        }
        await connectDB();
        // Check if agent exists and is verified
        const agent = await User.findOne({
            ...
            for (let i = 0; i < availability.length; i++) {
                for (let j = i + 1; j < availability.length; j++) {
                    if (availability[i].dayOfWeek === availability[j].dayOfWeek &&
                        doTimeSlotsOverlap(availability[i].startTime, availability[i].endTime, availability[j].startTime, availability[j].endTime)) {
                        return NextResponse.json({ error: 'Time slots cannot overlap on the same day' }, { status: 400 });
                    }
                }
            }
        }
        ...
        return NextResponse.json({ error: 'An error occurred while updating agent schedule' }, { status: 500 });
    }
});
