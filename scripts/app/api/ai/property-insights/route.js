 ...
 */
async function generatePropertyInsights(request, userId) {
    try {
        await connectDB();
        ...
        return NextResponse.json({ error: 'An error occurred while generating property insights' }, { status: 500 });
    }
}
// Export the route handler with authentication and subscription check
// Requires a subscription with AI tools access
export const POST = withAuthAndSubscription(generatePropertyInsights, 'use_ai', [UserType.OWNER, UserType.DEALER, UserType.BUYER], PlanType.STANDARD // Requiring at least STANDARD plan which has AI access
);
