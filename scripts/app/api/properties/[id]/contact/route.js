 ...
 */
async function getContactInfo(request, userId) {
    try {
        await connectDB();
        ...
        return NextResponse.json({ error: 'An error occurred while fetching contact information' }, { status: 500 });
    }
}
// Export the route handler with authentication and subscription check
export const GET = withAuthAndSubscription(getContactInfo, 'view_contact', [UserType.BUYER, UserType.OWNER, UserType.DEALER], PlanType.FREE);
