...
export async function GET(req) {
    try {
        ...
        if (currentYield)
            query.currentYield = Number(currentYield);
        if (name)
            query.name = { $regex: name, $options: "i" }; // case-insensitive match
        if (minPrice || maxPrice) {
            query.pricePerShare = {};
            if (minPrice)
                query.pricePerShare.$gte = Number(minPrice);
            if (maxPrice)
                query.pricePerShare.$lte = Number(maxPrice);
        }
        ...
        const properties = rawProperties.map((property) => (Object.assign(Object.assign({}, property), { id: property._id.toString() })));
