...
import { FaBuilding, FaChartLine, FaPercentage, FaSpinner, FaMapMarkerAlt, FaUsers, FaExclamationTriangle, FaSearch } from "react-icons/fa";
...
export default function CommercialListingsPage() {
    ...
    const fetchProperties = async () => {
        ...
        try {
            ...
            if (data.success && ((_a = data.data) === null || _a === void 0 ? void 0 : _a.properties)) {
    ...
    useEffect(() => {
        ...
        // Search filter
        if (searchTerm) {
            filtered = filtered.filter(property => property.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
        ...
        if (selectedLocation !== "all") {
            filtered = filtered.filter(property => property.address.city === selectedLocation ||
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-black">
            ...
            <CardContent className="p-6">
              <div className="flex flex-col lg:flex-row gap-4 items-center">
                ...
                <div className="relative flex-1">
                  ...
                  <Input placeholder="Search properties, locations..." value={searchTerm} onChange={(_e) => setSearchTerm(_e.target.value)} className="pl-10 bg-gray-800 border-gray-700 text-white"/>
                </div>
                ...
                <div className="flex flex-wrap gap-4 items-center">
                  <Select value={selectedType} onValueChange={setSelectedType}>
                    ...
                    <SelectContent className="bg-gray-800 border-gray-700">
                      {propertyTypeOptions.map((option) => (<SelectItem key={option.value} value={option.value} className="text-white">
                          {option.label}
                        </SelectItem>))}
                    </SelectContent>
                  </Select>
                  ...
                  <Select value={selectedLocation} onValueChange={setSelectedLocation}>
                    ...
                    <SelectContent className="bg-gray-800 border-gray-700">
                      {locationOptions.map((option) => (<SelectItem key={option.value} value={option.value} className="text-white">
                          {option.label}
                        </SelectItem>))}
                    </SelectContent>
                  </Select>
      ...
      <section className="px-4 pb-16">
        <div className="max-w-7xl mx-auto">
          ...
          {/* Error State */}
          {error && !isLoading && (<div className="text-center py-20">
              <Card className="luxury-card max-w-md mx-auto">
                <CardContent className="p-8">
                  ...
                  <Button onClick={fetchProperties} className="luxury-button">
                    Try Again
                  </Button>
                </CardContent>
              </Card>
          ...
          {/* Empty State */}
          {!isLoading && !error && filteredProperties.length === 0 && properties.length === 0 && (<div className="text-center py-20">
              <Card className="luxury-card max-w-md mx-auto">
                <CardContent className="p-8">
          ...
          {!isLoading && !error && filteredProperties.length === 0 && properties.length > 0 && (<div className="text-center py-20">
            ...
            }} variant="outline" className="luxury-button-outline">
                    Clear Filters
                  </Button>
                </CardContent>
              </Card>
          ...
          {/* Properties Grid */}
          {!isLoading && !error && filteredProperties.length > 0 && (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {filteredProperties.map((property, index) => (<motion.div key={property._id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: index * 0.1 }}>
                <Card className="luxury-card hover:shadow-orange-500/30 hover:shadow-xl transition-all duration-300 overflow-hidden">
                  {/* Property Image */}
                  <div className="relative h-64">
                    <Image src={property.images[0] || "/commercial/placeholder.jpg"} alt={property.title} fill className="object-cover"/>
                    {property.featured && (<Badge className="absolute top-4 left-4 bg-orange-500 text-white">
                        Featured
                      </Badge>)}
                    {property.status === 'sold_out' && (<Badge className="absolute top-4 right-4 bg-red-500 text-white">
                        Sold Out
                      </Badge>)}
                  ...
                  <CardContent className="p-6">
                    ...
                    <Link href={`/commercial/${property._id}`}>
                      <Button className="w-full luxury-button" disabled={property.status === 'sold_out'}>
