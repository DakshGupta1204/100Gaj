...
export default function BuySharesModal({ property, isOpen, onClose, onSuccess, }) {
    ...
    const handleShareCountChange = (value) => {
        ...
        if (shareCount <= property.availableShares && shareCount > 0) {
            setForm(Object.assign(Object.assign({}, form), { shareCount, totalAmount: shareCount * property.pricePerShare }));
        }
    };
    ...
    const handlePaymentSuccess = (_paymentResponse) => {
        setIsProcessing(false);
        setCurrentStep("details");
        toast.success(`Successfully purchased ${form.shareCount} shares of ${property.title}!`);
        onSuccess();
        onClose();
    };
    ...
    const getPaymentData = () => {
        ...
        return {
            ...
            userId: (userData === null || userData === void 0 ? void 0 : userData.id) || 'guest',
    ...
    return (<AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        {/* Modal */}
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto">
          <Card className="luxury-card">
            ...
            <CardContent className="space-y-6">
              {currentStep === "details" && (<>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <Input id="share-count" type="number" min="1" max={property.availableShares} value={form.shareCount} onChange={(_e) => handleShareCountChange(_e.target.value)} className="bg-gray-800 border-gray-700 text-white"/>
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <Input id="total-amount" type="number" value={form.totalAmount} onChange={(_e) => handleAmountChange(_e.target.value)} className="pl-8 bg-gray-800 border-gray-700 text-white"/>
                      </div>
                      <p className="text-xs text-gray-400 mt-1">
                  ...
                  <div className="space-y-4">
                    <div className="flex items-start space-x-3">
                      <Checkbox id="terms" checked={form.agreedToTerms} onCheckedChange={(checked) => setForm(Object.assign(Object.assign({}, form), { agreedToTerms: !!checked }))} className="mt-1"/>
                    ...
                    <div className="flex items-start space-x-3">
                      <Checkbox id="risk-disclosure" checked={form.agreedToRiskDisclosure} onCheckedChange={(checked) => setForm(Object.assign(Object.assign({}, form), { agreedToRiskDisclosure: !!checked }))} className="mt-1"/>
                      <Label htmlFor="risk-disclosure" className="text-sm text-gray-300 cursor-pointer">
              ...
              {currentStep === "payment" && (<div className="text-center py-8">
              ...
              {/* Action Buttons */}
              {currentStep === "details" && (<div className="flex space-x-4">
                  <Button variant="outline" onClick={onClose} className="flex-1 luxury-button-outline">
                    Cancel
                  </Button>
                  
                  <Button disabled={!canProceed() || isProcessing} className="flex-1 luxury-button" onClick={() => {
                ...
                handlePaymentSuccess({});
                    ...
                    {isProcessing ? (<div className="flex items-center">
                        ...
                        Processing...
                      </div>) : (`Purchase Shares - ${formatCurrency(form.totalAmount)}`)}
                  </Button>
                </div>)}
            </CardContent>
          </Card>
