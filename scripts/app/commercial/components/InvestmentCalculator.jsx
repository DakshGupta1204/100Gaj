...
export default function InvestmentCalculator({ property }) {
    ...
    const calculateReturns = (investment, shares) => {
    ...
    return (<Card className="luxury-card">
      <CardHeader>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="flex space-x-2">
          <button onClick={() => setInputMode('amount')} className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${inputMode === 'amount'
            ? 'bg-orange-500 text-white'
            : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`}>
            By Amount
          </button>
          <button onClick={() => setInputMode('shares')} className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${inputMode === 'shares'
            ? 'bg-orange-500 text-white'
            : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`}>
            By Shares
          </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <div className="relative">
              ...
              <Input id="investment-amount" type="number" placeholder={`Min: ${formatCurrency(property.minInvestment)}`} value={investmentAmount} onChange={(e) => setInvestmentAmount(e.target.value)} className="pl-8 bg-gray-800 border-gray-700 text-white" disabled={inputMode === 'shares'}/>
            </div>
            {investmentAmount && parseFloat(investmentAmount) < property.minInvestment && (<p className="text-red-400 text-sm mt-1">
          ...
          <div>
            ...
            <Input id="share-count" type="number" placeholder={`Max: ${property.availableShares}`} value={shareCount} onChange={(e) => setShareCount(e.target.value)} className="bg-gray-800 border-gray-700 text-white" disabled={inputMode === 'amount'}/>
            {shareCount && parseInt(shareCount) > property.availableShares && (<p className="text-red-400 text-sm mt-1">
        ...
        {/* Calculation Results */}
        {calculation && (<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="space-y-6">
            {/* Investment Summary */}
