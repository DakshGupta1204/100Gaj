...
import { FaArrowLeft, FaMapMarkerAlt, FaUsers, FaChartLine, FaPercentage, FaRupeeSign, FaStar, FaDownload, FaShieldAlt, FaCheckCircle, FaInfoCircle } from "react-icons/fa";
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8">
            ...
            <div className="mt-6 lg:mt-0">
              <Button onClick={handleBuyShares} size="lg" className="luxury-button px-8 py-3 text-lg" disabled={property.status === 'sold_out'}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 h-96 lg:h-[500px]">
            <div className="lg:col-span-2">
              <div className="relative w-full h-full rounded-lg overflow-hidden">
                <Image src={property.images[currentImageIndex] || "/commercial/placeholder.jpg"} alt={property.title} fill className="object-cover"/>
              </div>
            </div>
            <div className="grid grid-cols-2 lg:grid-cols-1 gap-4">
              {property.images.slice(0, 3).map((image, index) => (<div key={index} className={`relative h-32 lg:h-[160px] rounded-lg overflow-hidden cursor-pointer ${currentImageIndex === index ? 'ring-2 ring-orange-500' : ''}`} onClick={() => setCurrentImageIndex(index)}>
                  <Image src={image} alt={`${property.title} - ${index + 1}`} fill className="object-cover"/>
                  {currentImageIndex === index && (<div className="absolute inset-0 bg-orange-500/20"/>)}
                </div>))}
            </div>
          </div>
        </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                          ...
                          <div className="space-y-2">
                            {property.amenities.map((amenity, index) => (<div key={index} className="flex items-center text-gray-300">
                        ...
                        <div>
                          ...
                          <div className="space-y-2">
                            {property.nearbyLandmarks.map((landmark, index) => (<div key={index} className="flex items-center text-gray-300">
                  ...
                  <Card className="luxury-card">
                    <CardContent className="p-6">
                      ...
                      <div className="space-y-4">
                        {property.documents.map((doc, index) => (<div key={index} className="flex items-center justify-between p-4 bg-gray-800/50 rounded-lg">
                            <div className="flex items-center">
