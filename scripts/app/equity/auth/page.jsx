...
export default function AuthPage() {
    ...
    useEffect(() => {
        ...
        if ((callbackUrl === null || callbackUrl === void 0 ? void 0 : callbackUrl.includes("postproperty")) && !toastShownRef.current) {
    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            const errorMsg = ((_b = (_a = error.response) === null || _a === void 0 ? void 0 : _a.data) === null || _b === void 0 ? void 0 : _b.error) || error.message || "Invalid email or password";
            setError(errorMsg);
    ...
    const handleSignup = async (e) => {
        ...
        try {
            ...
            const errorMsg = ((_b = (_a = error.response) === null || _a === void 0 ? void 0 : _a.data) === null || _b === void 0 ? void 0 : _b.error) || "Signup failed. Please try again.";
            setError(errorMsg);
            toast.error(errorMsg);
        }
        finally {
    ...
    const renderInputField = (field) => (<div key={field.id} className="group">
      ...
      <div className="relative">
        <Input type={field.hasToggle ? (showPassword ? "text" : "password") : field.type} value={field.value} onChange={(e) => handleInputChange(field.id, e.target.value)} placeholder={field.placeholder} className="flex h-9 w-full rounded-md border px-3 py-1 text-base shadow-sm bg-transparent text-white placeholder-gray-500 border-[#a78bfa]/30 focus:outline-none focus:border-[#a78bfa] focus:ring-1 focus:ring-[#a78bfa] transition-colors duration-200"/>
        {field.hasToggle && (<Button type="button" variant="ghost" size="sm" onClick={() => setShowPassword(!showPassword)} className="absolute inset-y-0 right-0 pr-4 flex items-center text-gray-400 hover:text-[#a78bfa] transition-colors duration-200 hover:bg-transparent h-auto" tabIndex={-1}>
        ...
        <div className="max-w-lg mx-auto px-4 py-8">
          <div className="bg-[#14141F]/80 backdrop-blur-xl border-2 border-[#a78bfa] rounded-3xl shadow-2xl shadow-[#a78bfa]/20 overflow-hidden transform transition-all duration-300 hover:shadow-[#a78bfa]/30">
            ...
            : "text-gray-400 hover:text-[#a78bfa]"}`}>
              ...
              {/* Alerts */}
              {error && (<Alert className="mb-4 p-3 rounded-2xl border border-red-500/20 bg-red-500/10 text-red-400 text-sm backdrop-blur-sm">
                  <AlertCircle size={16}/>
                  <AlertDescription className="ml-2">{error}</AlertDescription>
                </Alert>)}
              {success && (<Alert className="mb-4 p-3 rounded-2xl border border-green-500/20 bg-green-500/10 text-green-400 text-sm backdrop-blur-sm">
                  <CheckCircle size={16}/>
              ...
              {/* Signup Form */}
              {activeTab === "signup" && (<form onSubmit={handleSignup} className="space-y-4">
                  <div className="space-y-4">
                    {signupFields.map(renderInputField)}
                  </div>

                  <div className="flex items-center space-x-3">
                    <Checkbox id="isInvestor" checked={formData.role === "investor"} onCheckedChange={(checked) => handleInputChange("role", checked ? "investor" : "owner")} className="w-4 h-4 rounded border-2 border-[#a78bfa]/40 text-[#a78bfa] focus:ring-[#a78bfa]/50 bg-black/50"/>
                    <Label htmlFor="isInvestor" className="text-gray-300 text-sm cursor-pointer">
