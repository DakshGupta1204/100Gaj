...
export default function BackgroundVideo({ className = "" }) {
        ...
        const createParticles = () => {
            for (let i = 0; i < 50; i++) {
                particles.push({
                    ...
                    type: ['data', 'chart', 'grid'][Math.floor(Math.random() * 3)]
                });
            }
        };
        // Draw grid lines
    ...
    return (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1 }} className={`fixed inset-0 pointer-events-none z-0 ${className}`}>
