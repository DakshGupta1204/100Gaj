...
import { ArrowLeft, MapPin, TrendingUp, Shield, Star, Users, Calendar, Target, Building2, Share2, Heart, CheckCircle, Plus, Minus, ShoppingCart, Briefcase, Warehouse, Store, Server, Coffee } from "lucide-react";
...
export default function PropertyDetailPage({ params }) {
    ...
    return (<div className="min-h-screen bg-black text-white">
      ...
      <div className="bg-gradient-to-b from-gray-900 to-black border-b border-gray-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex items-center gap-4 mb-6">
            <Button onClick={() => router.back()} variant="outline" size="sm" className="border-gray-600 text-gray-300 hover:bg-gray-800">
            ...
            <div className="flex items-center gap-3">
              <Button variant="outline" size="sm" onClick={() => setIsFavorite(!isFavorite)} className={`border-gray-600 ${isFavorite ? 'text-red-400 border-red-400' : 'text-gray-300'} hover:bg-gray-800`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="relative">
              <div className="aspect-video rounded-2xl overflow-hidden bg-gray-800">
                <img src={property.images[0] || "/images/placeholder-property.jpg"} alt={property.title} className="w-full h-full object-cover"/>
            ...
            {/* Property Overview */}
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-gray-900/50 backdrop-blur-sm rounded-2xl p-6 border border-gray-800">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-gray-900/50 backdrop-blur-sm rounded-xl p-6 border border-gray-800">
                ...
                <div className="space-y-3">
                  {["Prime Location", "High Occupancy", "A+ Tenants", "Modern Facilities", "24/7 Security", "Backup Power"].map((feature, index) => (<div key={index} className="flex items-center gap-3">
              ...
              <div className="bg-gray-900/50 backdrop-blur-sm rounded-xl p-6 border border-gray-800">
                ...
                <div className="space-y-3">
                  {property.amenities.map((amenity, index) => (<div key={index} className="flex items-center gap-3">
            ...
            {/* Key Tenants */}
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="bg-gray-900/50 backdrop-blur-sm rounded-xl p-6 border border-gray-800">
              <h3 className="text-lg font-bold text-white mb-4">Key Tenants</h3>
              <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                {["Microsoft India", "Accenture", "Deloitte", "EY", "KPMG"].map((tenant, index) => (<div key={index} className="text-center p-4 bg-gray-900 rounded-full shadow-sm border-2 border-[#B6FF3F] transition-colors duration-200 cursor-pointer hover:bg-gray-800">
            ...
            {/* Financial Analysis */}
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="bg-gray-900/50 backdrop-blur-sm rounded-xl p-6 border border-gray-800">
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-8 space-y-6">
              {/* Investment Panel */}
              <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.6 }} className="bg-gradient-to-br from-gray-900 to-black rounded-2xl p-6 border border-gray-800">
                ...
                <div className="mb-6">
                  ...
                  <div className="w-full bg-gray-800 rounded-full h-2 mb-2">
                    <div className="h-2 rounded-full bg-[#B6FF3F]" style={{ width: `${(property.availableShares / property.totalShares) * 100}%` }}/>
                  </div>
                ...
                <div className="mb-6">
                  <div className="flex justify-between items-center mb-3">
                    ...
                    <div className="flex items-center gap-2">
                      <Button size="sm" variant="outline" onClick={() => setSelectedShares(Math.max(1, selectedShares - 1))} className="border-gray-600 text-gray-300 hover:bg-gray-800">
                        <Minus className="w-4 h-4"/>
                      </Button>
                      <span className="text-lg font-bold text-white px-4">{selectedShares}</span>
                      <Button size="sm" variant="outline" onClick={() => setSelectedShares(Math.min(property.availableShares, selectedShares + 1))} className="border-gray-600 text-gray-300 hover:bg-gray-800">
                ...
                {/* Buy Button */}
                <Button onClick={handleBuyShares} className="w-full bg-[#d1ff4a] hover:bg-[#b6e944] text-black font-semibold py-3 rounded-lg transition-all duration-300 transform hover:scale-105">
              ...
              {/* Market Analysis */}
              <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.7 }} className="bg-gray-900/50 backdrop-blur-sm rounded-xl p-6 border border-gray-800">
              ...
              {/* Property Manager */}
              <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.8 }} className="bg-gray-900/50 backdrop-blur-sm rounded-xl p-6 border border-gray-800">
      ...
      <BuySharesModal property={property} isOpen={showBuyModal} onClose={() => setShowBuyModal(false)} onSuccess={handlePurchaseSuccess}/>
    </div>);
}
