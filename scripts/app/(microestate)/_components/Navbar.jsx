...
function Navbar() {
    ...
    return (<nav className="fixed top-0 left-0 right-0 z-50 bg-[#101014] shadow-lg border-b border-[#23232a]">
            <div className="container mx-auto px-8 py-3 flex items-center justify-between">
                ...
                <div className="hidden md:flex items-center space-x-2">
                            {navItems.map((item) => (<button key={item.name} onClick={() => router.push(item.path)} className={cn("flex items-center gap-2 px-5 py-2 rounded-lg font-medium transition-all duration-200", isActive(item.path)
                ...
                <div className="hidden md:block ml-8">
                            <Button className="bg-orange-500 hover:bg-orange-600 text-white font-bold rounded-lg px-6 py-2 shadow-md transition-all duration-200" onClick={() => router.push("/microestate/auth")}>
                                let&apos;s talk
                            </Button>
                        </div>
            </div>
