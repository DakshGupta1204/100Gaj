...
export default function PropertyListingPage() {
    ...
    // Filtering logic
    let filtered = properties.filter(p => (!location || p.location === location) &&
        ...
        <div className="filter-card bg-[#1e1e2f] bg-opacity-90 rounded-2xl p-6 mb-8 shadow-2xl relative backdrop-blur-md border border-orange-500/10">
          ...
          <div className="flex flex-wrap gap-6 mb-4">
            ...
            <div className="flex flex-col gap-2 min-w-[150px] flex-1">
              ...
              <select className="rounded-lg px-4 py-2 bg-[#23232a] border border-orange-400 text-white focus:ring-2 focus:ring-orange-500 transition" value={location} onChange={e => setLocation(e.target.value)}>
            ...
            <div className="flex flex-col gap-2 min-w-[170px] flex-1">
              ...
              <select className="rounded-lg px-4 py-2 bg-[#23232a] border border-orange-400 text-white focus:ring-2 focus:ring-orange-500 transition" value={sortBy} onChange={e => setSortBy(e.target.value)}>
          ...
          <div className="flex flex-col gap-2 mb-4 w-full">
            ...
            <div className="flex flex-wrap gap-2">
              {propertyTypes.map(type => (<button key={type.label} className={`pill flex items-center gap-1 px-4 py-2 rounded-full border transition-all duration-200 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-orange-500 ${selectedTypes.includes(type.label) ? 'bg-orange-500 text-white border-orange-500 shadow-lg' : 'bg-[#2c2c3e] text-white border-[#444] hover:bg-orange-400/30'}`} onClick={() => handleTypeChange(type.label)} type="button">
          ...
          <div className="flex flex-col gap-2 w-full">
            ...
            <div className="flex flex-wrap gap-2">
              {amenitiesList.map(am => (<button key={am.label} className={`pill flex items-center gap-1 px-4 py-2 rounded-full border transition-all duration-200 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-orange-500 ${selectedAmenities.includes(am.label) ? 'bg-orange-500 text-white border-orange-500 shadow-lg' : 'bg-[#2c2c3e] text-white border-[#444] hover:bg-orange-400/30'}`} onClick={() => handleAmenityChange(am.label)} type="button">
                  {am.icon} {am.label}
                </button>))}
            </div>
          </div>
