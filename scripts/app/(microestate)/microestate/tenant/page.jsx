...
export default function TenantDashboard() {
    ...
    // Filtering logic
    let filtered = allProperties.filter(p => (!location || p.location === location) &&
        (!minPrice || p.price >= Number(minPrice)) &&
        (!maxPrice || p.price <= Number(maxPrice)) &&
        (!minSize || p.size >= Number(minSize)) &&
        (selectedTypes.length === 0 || selectedTypes.includes(p.type)) &&
        (selectedAmenities.length === 0 || selectedAmenities.every(a => { var _a; return (_a = p.amenities) === null || _a === void 0 ? void 0 : _a.includes(a); })));
    if (sortBy === 'priceLow')
    ...
    const filteredInquiries = myInquiries.filter(i => inquiryFilter === 'All' ? true :
    ...
    return (<div className="relative min-h-screen overflow-x-hidden">
      ...
      <div className="container mx-auto py-10 mt-24 relative z-10">
        {/* Welcome Banner */}
        <motion.div className="mb-8" initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.6 }} transition={{ duration: 0.7, ease: 'easeOut' }}>
        ...
        <motion.div className="bg-[#181c24] rounded-xl p-6 mb-8 flex flex-wrap gap-4 items-end shadow-lg" initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.6 }} transition={{ duration: 0.7, delay: 0.1, ease: 'easeOut' }}>
          {/* Location Dropdown */}
          <div>
            <label className="block text-sm text-gray-300 mb-1">Location</label>
            <select className="w-40 rounded px-3 py-2 bg-[#23232a] text-white border border-orange-500/20 focus:border-orange-500" value={location} onChange={e => setLocation(e.target.value)}>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {propertyTypes.map(type => (<label key={type} className={`px-3 py-1 rounded-full cursor-pointer border text-xs ${selectedTypes.includes(type) ? 'bg-orange-500 text-white border-orange-500' : 'bg-[#23232a] text-gray-300 border-orange-500/20'}`}>
                  <input type="checkbox" checked={selectedTypes.includes(type)} onChange={() => handleTypeChange(type)} className="mr-1 accent-orange-500"/>
                  {type}
                </label>))}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {amenitiesList.map(am => (<label key={am} className={`px-3 py-1 rounded-full cursor-pointer border text-xs ${selectedAmenities.includes(am) ? 'bg-orange-500 text-white border-orange-500' : 'bg-[#23232a] text-gray-300 border-orange-500/20'}`}>
                  <input type="checkbox" checked={selectedAmenities.includes(am)} onChange={() => handleAmenityChange(am)} className="mr-1 accent-orange-500"/>
                  {am}
                </label>))}
            </div>
          </div>
          ...
          <div>
            ...
            <select className="w-40 rounded px-3 py-2 bg-[#23232a] text-white border border-orange-500/20 focus:border-orange-500" value={sortBy} onChange={e => setSortBy(e.target.value)}>
              {sortOptions.map(opt => <option key={opt.value} value={opt.value}>{opt.label}</option>)}
            </select>
          </div>
        </motion.div>
        {/* Property Listings Grid */}
        <motion.div className="mb-10" initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.6 }} transition={{ duration: 0.7, delay: 0.2, ease: 'easeOut' }}>
          <h2 className="text-3xl font-bold heading-gradient mb-8">Browse Properties</h2>
          <div className="section">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {filtered.map(prop => {
            var _a, _b;
            return (<div key={prop.id} className="card-glass flex flex-col items-center border border-gray-700 rounded-2xl transition-all duration-300 hover:scale-105 hover:-translate-y-2 hover:border-orange-400">
                  ...
                  <div className="flex gap-1 mb-1">
                    {(_a = prop.badges) === null || _a === void 0 ? void 0 : _a.map(badge => (<span key={badge} className="px-2 py-0.5 text-xs rounded bg-orange-500 text-white font-bold mr-1">{badge}</span>))}
                  </div>
                  <div className="flex flex-wrap gap-2 mb-2">
                    {(_b = prop.amenities) === null || _b === void 0 ? void 0 : _b.map(am => (<span key={am} className="px-2 py-1 text-xs rounded bg-[#181c24] text-gray-200">{am}</span>))}
                  </div>
        ...
        {/* Saved/Favorite Properties */}
        <motion.div className="mb-10" initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.6 }} transition={{ duration: 0.7, delay: 0.3, ease: 'easeOut' }}>
          ...
          <div className="flex gap-4 overflow-x-auto pb-2">
            ...
            {favoriteProperties.map(prop => (<div key={prop.id} className="min-w-[260px] bg-[#23232a] rounded-xl shadow-md p-4 flex flex-col items-center">
        ...
        {/* Inquiries & Responses */}
        <motion.div className="mb-10" initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.6 }} transition={{ duration: 0.7, delay: 0.4, ease: 'easeOut' }}>
          <h2 className="text-lg font-bold text-white mb-4">My Inquiries & Responses</h2>
          <div className="flex gap-4 mb-2">
            {['All', 'Awaiting response', 'Replied'].map(f => (<button key={f} className={`px-4 py-1 rounded-full text-sm font-semibold ${inquiryFilter === f ? 'bg-orange-500 text-white' : 'bg-[#23232a] text-gray-300 border border-orange-500/20'}`} onClick={() => setInquiryFilter(f)}>{f}</button>))}
          </div>
          <div className="bg-[#23232a] rounded-xl overflow-x-auto">
            <table className="min-w-full text-sm text-left">
              ...
              <tbody>
                {filteredInquiries.map((inq, idx) => (<tr key={idx} className="border-b border-[#181c24] last:border-0">
        ...
        <motion.div className="mb-10" initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.6 }} transition={{ duration: 0.7, delay: 0.5, ease: 'easeOut' }}>
